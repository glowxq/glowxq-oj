...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public LoginVO autoWechatLogin(String wechatCode) {
        ClientVO client = clientService.getByClientId("");
        LoginInfo loginInfo = new LoginInfo();
        loginInfo.setCode(wechatCode);
        loginInfo.setGrantType(GrantType.wechat_applet.getCode());
        loginInfo.setAutoRegister(true);
        return IAuthStrategy.login(loginInfo, client, loginInfo.getGrantType());
    }

    @Override
    public LoginVO loginTest(String username) {
        ClientVO client = clientService.getByClientId("195da9fcce574852b850068771cde034");
        LoginInfo loginInfo = new LoginInfo();
        loginInfo.setUsername(username);
        loginInfo.setGrantType(GrantType.test.getCode());
        return IAuthStrategy.login(loginInfo, client, loginInfo.getGrantType());
    }
}
