...
public class OssClient {
    ...
    public void initBucket() {
        ...
        try {
            ...
            log.warn("设置 Bucket CORS 失败 (可忽略): {}", e.getMessage());
        }
        try {
            ...
            log.warn("设置 Bucket ACL 失败 (MinIO 不支持此操作，可忽略): {}", e.getMessage());
        }
    }
    ...
    public void createBucket() {
        try {
            ...
            // Bucket 已存在，跳过创建
            log.info("OSS Bucket [{}] 已存在，跳过创建", properties.getBucketName());
        }
    }
