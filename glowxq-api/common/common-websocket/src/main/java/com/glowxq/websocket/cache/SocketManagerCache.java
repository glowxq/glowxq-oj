package com.glowxq.websocket.cache;
...
public class SocketManagerCache {
    ...
    public static WsSession getOnlineSession(String sessionId) {
        return onlineSessionMap.get(sessionId);
    }

    public static void addOnlineSid(String loginId, String sessionId) {
        if (onlineUserSessionIdMap.containsKey(loginId)) {
            List<String> sidArray = onlineUserSessionIdMap.get(loginId);
            sidArray.add(sessionId);
    ...
    public static void removeUserSession(String sessionId) {
        WsSession wsSession = onlineSessionMap.get(sessionId);
