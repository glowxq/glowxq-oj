...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public SysUserRoleVO findSysUserRole(Long userId) {
        List<SysRole> sysRoleList = sysRoleService.listIgnoreSuperAdmin();
    ...
    public SysUser register(RegisterDTO registerParam) {
        ...
        if (StringUtils.isNotBlank(registerParam.getWechatCode())) {
            LoginInfoResult loginInfoResult = wechatService.login(registerParam.getWechatCode());
            user.setOpenid(loginInfoResult.getOpenid());
        }
    ...
    @Override
    public void bindWechat(String wechatLoginCode) {
        LoginInfoResult loginInfoResult = wechatService.login(wechatLoginCode);
        SysUser user = mapper.selectOneById(LoginUtils.getUserId());
        user.setOpenid(loginInfoResult.getOpenid());
        mapper.update(user);
    }

    @Override
    public LoginUser autoWechatLogin(String openid, Boolean autoRegister) {
        // register()
        return null;
    }
