...
import com.glowxq.system.admin.mapper.SysDeptMapper;
...
import com.glowxq.system.admin.service.SysDeptClosureService;
...
import static com.glowxq.system.admin.pojo.po.table.SysDeptLeaderTableDef.SYS_DEPT_LEADER;
...
import static com.glowxq.system.admin.pojo.po.table.SysUserTableDef.SYS_USER;
...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements SysDeptService {
    ...
    public List<SysDeptVO> list(SysDeptListDTO dto) {
        QueryWrapper wrapper = QueryWrapper.create().select(SYS_DEPT.ALL_COLUMNS, QueryMethods.groupConcat(
                                                   if_(SYS_USER.DEL_FLAG.eq("F"), QueryMethods.concatWs(QueryMethods.string(":"), SYS_DEPT_LEADER.LEADER_ID,
    ...
    private void setUseTotal(List<DeptTreeVO> deptTreeVOS) {
        // 查询直属部门的数量
        QueryWrapper wrapper = QueryWrapper.create()
                                           .select(SYS_DEPT.ID, SYS_DEPT.NAME, QueryMethods.count(case_().when(SYS_USER.DEL_FLAG.eq("F")).then(SYS_USER_DEPT.USER_ID)
                                                                                                         .end()).as("total"))
                                           .from(SYS_DEPT).leftJoin(SYS_USER_DEPT).on(SYS_DEPT.ID.eq(SYS_USER_DEPT.DEPT_ID)).leftJoin(SYS_USER)
                                           .on(SYS_USER_DEPT.USER_ID.eq(SYS_USER.ID))
    ...
    @Override
    public List<SysDept> searchDeptList(String searchKey, Long regionId) {
        if (StringUtils.isBlank(searchKey) && regionId == null) {
            return new ArrayList<>();
        }
        QueryWrapper wrapper = QueryWrapper.create();
        wrapper.limit(5);
        if (StringUtils.isNotBlank(searchKey)) {
            wrapper.or(qw -> {qw.like(SysDept::getContent, searchKey);});
            wrapper.or(qw -> {qw.like(SysDept::getDeptNumber, searchKey);});
            wrapper.or(qw -> {qw.like(SysDept::getName, searchKey);});
            wrapper.or(qw -> {qw.like(SysDept::getPhone, searchKey);});
        }
        if (regionId != null) {
            wrapper.eq(SysDept::getRegionId, regionId);
        }
