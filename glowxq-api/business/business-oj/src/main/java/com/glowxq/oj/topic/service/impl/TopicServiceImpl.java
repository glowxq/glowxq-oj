...
public class TopicServiceImpl extends ServiceImpl<TopicMapper, Topic> implements TopicService {
    ...
    public void create(TopicCreateDTO dto) {
        ...
        this.save(topic);
        groupService.bindGroups(topic.getId(), dto.getGroupIds(), OjTagBind.Topic);
    ...
    public void update(TopicUpdateDTO dto) {
        ...
        // 解绑数据
        groupService.unBindAll(topic.getId(), OjTagBind.Topic);
    ...
    public PageResult<TopicVO> page(TopicListDTO dto) {
        ...
        if (CollectionUtils.isNotEmpty(dto.getGroupIds()) && CollectionUtils.isNotEmpty(dto.getTagIds())) {
            throw new BusinessException("班级、标签 两个筛选条件不能同时选择");
        }
        List<Long> topicIdsByGroup = groupService.listBusinessIdByGroupIds(dto.getGroupIds(), OjTagBind.Topic);
        List<Long> topicIdsByTag = metaTagService.listBusinessIdByTagIds(dto.getTagIds(), OjTagBind.Topic);
        queryWrapper.in(Topic::getId, topicIdsByGroup, CollectionUtils.isNotEmpty(topicIdsByGroup));
    ...
    public void remove(SelectIdsDTO dto) {
        ...
        groupService.unBindsAll(dto.getIds(), OjTagBind.Topic);
    ...
    public TopicVO detail(Long id) {
        ...
        List<Group> groups = groupService.listByBusinessId(topicVO.getId(), OjTagBind.Topic);
