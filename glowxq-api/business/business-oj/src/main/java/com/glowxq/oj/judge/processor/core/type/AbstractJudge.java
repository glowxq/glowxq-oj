package com.glowxq.oj.judge.processor.core.type;
...
import com.glowxq.oj.judge.processor.bo.JudgeDTO;
import com.glowxq.oj.judge.processor.bo.JudgeGlobalDTO;
import com.glowxq.oj.judge.processor.bo.SandBoxRes;
    ...
    protected static List<String> parseRunCommand(String command,
        ...
        if (testCaseInputName != null) {
            command = command.replace("{std_input}",
                    JudgeDir.TMPFS_DIR.getContent() + File.separator + testCaseInputName);
        }

        if (userOutputName != null) {
            command = command.replace("{user_output}",
                    JudgeDir.TMPFS_DIR.getContent() + File.separator + userOutputName);
        }

        if (userOutputName != null) {
            command = command.replace("{std_output}",
                    JudgeDir.TMPFS_DIR.getContent() + File.separator + testCaseOutputName);
        }

        return JudgeUtils.translateCommandline(command);
    }

    public JSONObject judge(JudgeDTO judgeDTO, JudgeGlobalDTO judgeGlobalDTO) {
        ...
        switch (judgeGlobalDTO.getJudgeMode()) {
            ...
            default:
                throw new BusinessException("The problem mode is error:" + judgeGlobalDTO.getJudgeMode());
        }
    }

    public abstract JSONArray judgeCase(JudgeDTO judgeDTO, JudgeGlobalDTO judgeGlobalDTO);
