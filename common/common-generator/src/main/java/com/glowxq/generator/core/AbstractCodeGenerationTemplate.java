...
public abstract class AbstractCodeGenerationTemplate {
    ...
    public CodeGenTempResult buildTemplate(boolean isSaveToLocal, Boolean overwrite) throws IOException {
        ...
        if (isSaveToLocal) {
            ...
            outputMsg = saveToFile(outputPath, template, model,overwrite);
        } else {
     ...
     *
     * @param outputPath 文件的输出路径，指定生成的文件保存的位置
     * @param template   使用的模板，用于生成文件的内容
     * @param model      数据模型，填充模板中的占位符
     * @param overwrite
     */
    private static String saveToFile(String outputPath, Template template, Map<String, Object> model, Boolean overwrite) {
        try {
